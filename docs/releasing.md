...
1. Confirm the version matches in `pyproject.toml`, `server.json`, `uv.lock`, the changelog heading, the pinned `uvx --from stremio-mcp-server==<version>` install example in `README.md`, and the current-release row of the `README.md` availability table.
